...
def init_connection():
    account = 'qbzotta-uo72260'
    user = 'CHANCHONIP'
    password = '3Kasj2136agt'
...
def show_input_page():
    with DataSection:
        if st.session_state['Data']:
            list1 = ['Course Name', 'Description', 'Prerequisites', 'Textbook', 'Reference', 'Major', 'Objectives',
                     'Covered', 'Objectives Outcomes']
            list2 = [st.session_state['csn'], st.session_state['des'], st.session_state['pre'], st.session_state['tex'],
                     st.session_state['ref'], st.session_state['maj'], st.session_state['obj'], st.session_state['cov'],
                     st.session_state['obo']]
            l1 = {list2[1]: list2}
            ...
            sql = "UPDATE course SET description = '%s', prerequisites = '%s', textbook = '%s', reference = '%s', " \
                  "major_pre = " \
                  "'%s', objectives = '%s', covered = '%s', objectives_outcomes = '%s' WHERE id_name = '%s'" % (
                  st.session_state['des'],
...
def show_fill_page():
    if st.session_state['main'] == False and st.session_state['Fill'] == True:
        with FillSection:
            ...
            with right_column:
                list123 = [catalog_description, prerequisites, textbook, references, major_prerequisites_by_topic,
                           course_objectives, topics_covered, objectives_and_outcomes, course_code]
